...
* **REST:** Unity posts `/session/new`, `/ask`, and `/health` to `ITSClient._apiUrl` (default `https://turing.erwinlabs.dev`). `/ask` body is `student_id`, `level_id`, `question` (snake_case Newtonsoft / `ItsRestJson`). Reply is `reply`; Unity does Wit TTS. If `/ask` cannot reach the host, `ITSClient` raises a successful `AskResult` with `O rádio não ta muito bom.`
