...
* **REST:** Unity posts `/session/new`, `/ask`, and `/health`. `/ask` body is `student_id`, `level_id`, `question` (snake_case Newtonsoft / `ItsRestJson`). Reply `audio_url` is always null; Unity does Wit TTS.
